...
| Incapacitation | 1.10.0 (usable from 1.11.0 - see below; knockdown added in 1.31.0) | `KnockDown()`, `KnockDownRelease()`, `IsKnockedDown()`, `GetKnockDownAvailability()` - the fall and the stand-up, one call each way, since 1.31.0. `KnockoutActor()`, `WakeActor()`, `KnockoutFall()` and `KnockoutRecover()` (these two deprecated since 1.31.0), `IsManagedUnconscious()`, `GetActorLifeState()`, `RegisterForActorWoke()`, `UnregisterForActorWoke()`, `RegisterForActorWokeAlias()`, `UnregisterForActorWokeAlias()`, event `OnActorWoke` |
...
**Since 1.31.0, the fall has its own four calls, and they replace the pair described below.** `KnockDown(akTarget, akSource)` drops the actor and holds it on the ground; `KnockDownRelease(akTarget)` stands it back up and dispatches `OnActorWoke`. You never call `Actor.SetUnconscious` yourself on this path. How long the actor stays down is yours to decide - there is no timer. A game load stands every knocked-down actor back up, with no `OnActorWoke`. `KnockDown` returns a code: zero or positive is success, negative is a refusal with its reason, and `GetKnockDownAvailability()` tells up front when the whole path is unavailable - for instance while Knockout Extensions is installed, since both rewrite the same engine call. Not available in VR. Gate on `Lodestone.GetVersion() >= 1031000`; the full contract and the code table are in `Lodestone.psc`. `KnockoutFall()` and `KnockoutRecover()` still work as before and are deprecated.
